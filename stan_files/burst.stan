...
#include functions.stan
  ...
  real intplus(int j, vector y, int numofki67int, int numofbrduint, int start) 
  {
    ...
    { // BRDU labeling of A and B
        ...
        //A++
        dy[1] = Source - (delta_A+gamma+numki67comp*beta+alpha_A)*y[1] + 2*alpha_A*eff*sum(y[1:switch1])+ 2*alpha_B*eff*sum(y[(switch1+1):]);
        
        //Aint+
        for (j in 0:(numofbrduint-1)) {
            dy[2+j] = - (delta_A+gamma+numki67comp*beta+alpha_A)*y[2+j]+2*alpha_A*(1-eff)*intplus(j,y,numofki67int,numofbrduint,1)+ 2*alpha_B*(1-eff)*intplus(j,y,numofki67int,numofbrduint,switch1+1);
        }
        
        //A-+
        dy[2+numofbrduint] = -(delta_A+gamma+alpha_A+numki67comp*beta)*y[2+numofbrduint] +2*alpha_A*(1-eff)*(intplus(numofbrduint,y,numofki67int,numofbrduint,1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,1))+2*alpha_B*(1-eff)*(intplus(numofbrduint,y,numofki67int,numofbrduint,switch1+1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,switch1+1));
        ...
        //Bint+
        for (j in 0:(numofbrduint-1)) {
            dy[switch1+2+j] = - (delta_B+numki67comp*beta+alpha_B)*y[switch1+2+j] + gamma*y[2+j] ;
        }
         
        //B-+
        dy[switch1+2+numofbrduint] = -(delta_B+numki67comp*beta+alpha_B)*y[switch1+2+numofbrduint]+gamma*y[2+numofbrduint] ;
    ...
    { // Unlabeling of BRDU
        ...
        //real exp_decay_value = 1-1/(1+exp(-1*(t-7-switchtime)));
        ...
        //Aint+
        for (j in 0:(numofbrduint-1)) {
            dy[2+j] = - (delta_A+gamma+numki67comp*beta+alpha_A)*y[2+j] + 2*alpha_A*intplus(j,y,numofki67int,numofbrduint,1) + 2*alpha_B*intplus(j,y,numofki67int,numofbrduint,switch1+1);
        }
         
        //A-+
        dy[2+numofbrduint] = Source*(1-exp_decay_value)-(delta_A+gamma+alpha_A+numki67comp*beta)*y[2+numofbrduint] + 2*alpha_A*(intplus(numofbrduint,y,numofki67int,numofbrduint,1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,1))+2*alpha_B*(intplus(numofbrduint,y,numofki67int,numofbrduint,switch1+1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,switch1+1));
    ...
    { // Calculating the steady state values
        ...
        //A-+
        dy[2+numofbrduint] = Source - (delta_A+gamma+alpha_A+numki67comp*beta)*y[2+numofbrduint] + 2*alpha_A*sum(y[:(switch1)])+2*alpha_B*sum(y[(switch1+1):]);
