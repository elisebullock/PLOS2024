...
#include functions.stan
  ...
  vector KIAB_labeling(real t,  vector y,
   ...
   { // BRDU labeling of A and B
        ...
        //A++
        dy[1] = Source - (delta_A+gamma+numki67comp*beta+alpha_A)*y[1] + 2*alpha_A*eff*sum(y[1:switch1]);
        
        //Aint+
        for (j in 0:(numofbrduint-1)) {
            dy[2+j] = - (delta_A+gamma+numki67comp*beta+alpha_A)*y[2+j]+2*alpha_A*(1-eff)*intplus(j,y,numofki67int,numofbrduint,1);
        }
        
        //A-+
        dy[2+numofbrduint] = -(delta_A+gamma+alpha_A+numki67comp*beta)*y[2+numofbrduint] +2*alpha_A*(1-eff)*(intplus(numofbrduint,y,numofki67int,numofbrduint,1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,1));
            
        //A(+ int -)int 
        for (i in 0:(numofki67int-1)) {
            for (j in 0:(numofbrduint+1)) {
                dy[(2+numofbrduint)*(1+i)+j+1] = -(delta_A+gamma+alpha_A+numki67comp*beta)*y[(2+numofbrduint)*(1+i)+j+1]+numki67comp*beta*y[(2+numofbrduint)*(i)+j+1];
            }
        }
            
        //A+-
        dy[(1+numofki67int)*(2+numofbrduint)+1] = -(delta_A+gamma+alpha_A)*y[(1+numofki67int)*(2+numofbrduint)+1]+numki67comp*beta*y[(numofki67int)*(2+numofbrduint)+1];
        
        //Aint-
        for (j in 0:(numofbrduint-1)) {
            dy[(1+numofki67int)*(2+numofbrduint)+2+j] = -(delta_A+gamma+alpha_A)*y[(1+numofki67int)*(2+numofbrduint)+2+j]+numki67comp*beta*y[(numofki67int)*(2+numofbrduint)+2+j];
        }
        
        //A--
        dy[switch1]  = -(delta_A+gamma+alpha_A)*y[switch1]+numki67comp*beta*y[(2+numofbrduint)*(1+numofki67int)];
         
        //B++
        dy[switch1+1] =   -(delta_B+numki67comp*beta+alpha_B)*y[switch1+1] + 2*alpha_B*eff*sum(y[(switch1+1):]) + gamma*y[1]; 
        
        //Bint+
        for (j in 0:(numofbrduint-1)) {
            dy[switch1+2+j] = - (delta_B+numki67comp*beta+alpha_B)*y[switch1+2+j] + gamma*y[2+j] + 2*alpha_B*(1-eff)*intplus(j,y,numofki67int,numofbrduint,switch1+1);
         }
         
        //B-+
        dy[switch1+2+numofbrduint] = -(delta_B+numki67comp*beta+alpha_B)*y[switch1+2+numofbrduint]+gamma*y[2+numofbrduint] +2*alpha_B*(1-eff)*(intplus(numofbrduint,y,numofki67int,numofbrduint,switch1+1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,switch1+1));
        
        //B(+ int -)int
        for (i in 0:(numofki67int-1)) {
            for (j in 0:(numofbrduint+1)) {
                dy[switch1+(2+numofbrduint)*(1+i)+j+1] = -(delta_B+alpha_B+numki67comp*beta)*y[switch1+(2+numofbrduint)*(1+i)+j+1]+numki67comp*beta*y[switch1+(2+numofbrduint)*(i)+j+1]+gamma*y[(2+numofbrduint)*(1+i)+j+1];
            }
        }
        
        //B+-
        dy[switch1+(1+numofki67int)*(2+numofbrduint)+1] = -(delta_B+alpha_B)*y[switch1+(1+numofki67int)*(2+numofbrduint)+1] + numki67comp*beta*y[switch1+(numofki67int)*(2+numofbrduint)+1] + gamma*y[(1+numofki67int)*(2+numofbrduint)+1];
        
        //Bint-
        for (j in 0:(numofbrduint-1)) {
            dy[switch1+(1+numofki67int)*(2+numofbrduint)+2+j] = -(delta_B+alpha_B)*y[switch1+(1+numofki67int)*(2+numofbrduint)+2+j]+numki67comp*beta*y[switch1+(numofki67int)*(2+numofbrduint)+2+j] + gamma*y[(1+numofki67int)*(2+numofbrduint)+2+j];
        }
        
        //B--
        dy[2*switch1] = -(delta_B+alpha_B)*y[2*switch1]+numki67comp*beta*y[(2+numofbrduint)*(1+numofki67int)+switch1]+gamma*y[switch1];
  ...
  { // Unlabeling of BRDU
          ...
          //A++
          dy[1] = Source*exp_decay_value - (delta_A+gamma+numki67comp*beta+alpha_A)*y[1];
          
          //Aint+
          for (j in 0:(numofbrduint-1)) {
              dy[2+j] = - (delta_A+gamma+numki67comp*beta+alpha_A)*y[2+j]+ 2*alpha_A*intplus(j,y,numofki67int,numofbrduint,1);
          }
         
          //A-+
          dy[2+numofbrduint] = Source*(1-exp_decay_value)-(delta_A+gamma+alpha_A+numki67comp*beta)*y[2+numofbrduint] + 2*alpha_A*(intplus(numofbrduint,y,numofki67int,numofbrduint,1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,1));
          ...
          //B++
          dy[switch1+1] = - (delta_B+numki67comp*beta+alpha_B)*y[switch1+1] + gamma*y[1];
          
          //Bint+
          for (j in 0:(numofbrduint-1)) {
              dy[switch1+2+j] = - (delta_B+numki67comp*beta+alpha_B)*y[switch1+2+j] + gamma*y[2+j] + 2*alpha_B*intplus(j,y,numofki67int,numofbrduint,switch1+1);
          }
          
          //B-+
          dy[switch1+2+numofbrduint] =  - (delta_B+numki67comp*beta+alpha_B)*y[switch1+2+numofbrduint]+gamma*y[2+numofbrduint]+2*alpha_B*(intplus(numofbrduint,y,numofki67int,numofbrduint,switch1+1)+intplus(numofbrduint+1,y,numofki67int,numofbrduint,switch1+1));
  ...
  vector equilibrium_ode(real t,  vector y,
    ...
    { // Calculating the steady state values
        ...
        //A-+
        dy[2+numofbrduint] = Source - (delta_A+gamma+alpha_A+numki67comp*beta)*y[2+numofbrduint] + 2*alpha_A*sum(y[:(switch1)]);
        ...
        //B-+
        dy[switch1+2+numofbrduint] =  -(delta_B+numki67comp*beta+alpha_B)*y[switch1+2+numofbrduint]+gamma*y[2+numofbrduint]+2*alpha_B*sum(y[(switch1+1):]);
    ...
    { // Calculating the steady state values
        ...
        // compute derivatives           
        //Fast
        dy[1] = -(delta_A+gamma-alpha_A)*y[1];
   
        //Slow
        dy[2] = -(delta_B-alpha_B)*y[2]+gamma*y[1];
  ...
  array[] vector solveode(vector init_cond, array[] real time_index, array[] real time_index_equilibrium,
        ...
        )
    {
        ...
        array[numObs] vector[2*switch1] k_hat; //= ode_rk45(KIAB_labeling, init_conditions[1], 0.0, time_index, alpha_A, alpha_B, delta_A, delta_B, beta, gamma, Source, mu,eff, numofki67int,numofbrduint);
                
        k_hat[1:index] = ode_rk45(KIAB_labeling, init_cond, 0.0, time_index[1:index], alpha_A, alpha_B, delta_A, delta_B, beta, gamma, Source, mu,eff, numofki67int,numofbrduint);
        k_hat[(index+1):numObs] = ode_rk45(KIAB_unlabeling, k_hat[index], switchtime, time_index[(index+1):numObs], alpha_A, alpha_B, delta_A, delta_B, beta, gamma, Source, mu,eff, numofki67int,numofbrduint, switchtime);
...
parameters{
  ...
  transformed parameters{
    ...
    array[1] vector[2*switch1] init_conditions = ode_bdf(equilibrium_ode, init_cond_obs, 0.0, time_index_equilibrium, alpha_A, alpha_B, delta_A, delta_B, beta, gamma, Source, mu,eff, numofki67int,numofbrduint);
...
generated quantities{
    ...
    real ppd_gamma = lognormal_rng(log(gamma_data), 1);
