...
sm = cmdstanpy.CmdStanModel(stan_file=paras.stanloc+"burst.stan")
...
cmdstanpy.write_stan_json(paras.filelocation+"burst.json", data_dict)

sam = sm.sample(
    data=paras.filelocation+"burst.json", 
    chains=5, 
    parallel_chains=5,
    refresh=5,
    inits=init_d,
...
import pickle
with open(paras.filelocation+"burst.pkl", 'wb') as f:
    pickle.dump(sam, f)
